...
优化器通常也包含参数. 优化器需要用计算图和模型参数初始化这些参数. 优化器参数和模型参数对应.

之后, 优化器接收梯度, 应用具体的优化算法, 更新模型参数和优化器参数.
...
| 配置名 | 默认值 | 含义 |
| - | - | - |
| ada_grad_alpha | 0.01 | 同adagrad优化器的alpha |
| ada_grad_beta | 1e-5 | 同adagrad优化器的beta |
| ftrl_alpha | 0.01 | 同ftrl优化器的alpha |
| ftrl_beta | 1 | 同ftrl优化器的beta |
| ftrl_l1 | 1 | 同ftrl优化器的l1 |
| ftrl_l2 | 0 | 同ftrl优化器的l2 |
...
| ada_grad_alpha | 0.01 | 同adagrad优化器的alpha |
| ada_grad_beta | 1e-5 | 同adagrad优化器的beta |
| gftrl_alpha | 0.01 | 同gftrl优化器的alpha |
| gftrl_beta | 1 | 同gftrl优化器的beta |
| gftrl_lambda | 1 | 同gftrl优化器的lambda |
...
| - | - | - |
| alpha | 0.01 | 学习率 |
| min\_alpha | 1e-6 | 学习率衰减相关 |
| batch\_decay | 128 | 学习率衰减相关 |
| batch\_decay\_rate | 0.95 | 学习率衰减相关 |
