...
class WePSSGDOptimizer : public WePSOptimizerBase0 {
 private:
  ll_we_ps_optimizer_t::SGDConfig config_;

 public:
  DEFINE_WE_PS_OPTIMIZER_LIKE(WePSSGDOptimizer);
 ...
 protected:
  ...
  void UpdateTSR2TSR(const std::string& /*name*/, const tsr_t& G,
    ...
    ll_we_ps_optimizer_t::UpdateTSR2TSR(config_, G, W, D);
  }

  void UpdateSRM2TSR(const std::string& /*name*/, const srm_t& G,
                     const tsr_t& W, tsr_t* D,
                     WePSOptimizerTSRSlot* /*slot*/) const override {
    ll_we_ps_optimizer_t::UpdateSRM2TSR(config_, G, W, D);
  }

  void UpdateSRM2SRM(const std::string& /*name*/, const srm_t& G,
                     const srm_t& W, srm_t* D,
                     WePSOptimizerSRMSlot* /*slot*/) const override {
    ll_we_ps_optimizer_t::UpdateSRM2SRM(config_, G, W, D);
  }
};

WE_PS_OPTIMIZER_REGISTER(WePSSGDOptimizer, "WePSSGDOptimizer");
WE_PS_OPTIMIZER_REGISTER(WePSSGDOptimizer, "sgd");
