...
struct Group18EmbeddingLookupAux {
  Shape Z;
  std::vector<int> Zoffset;  // indexed by group id

  int GetZoffset(int group_id) const noexcept {
...
bool Group18EmbeddingLookupPrepare(int Xrow, const std::vector<int>& group_ids,
                                   const std::vector<const Shape*>& W,
                                   Group18EmbeddingLookupAux* aux) {
  ...
  int max_group_id = *std::max_element(group_ids.begin(), group_ids.end()) + 1;
  int n = 0;
...
bool Group18EmbeddingLookupInferShape(int Xrow,
                                      const std::vector<int>& group_ids,
                                      const std::vector<const Shape*>& W,
                                      Shape* Z) {
  Group18EmbeddingLookupAux aux;
  if (!Group18EmbeddingLookupPrepare(Xrow, group_ids, W, &aux)) {
    return false;
  }
  *Z = aux.Z;
  return true;
...
void Group18EmbeddingLookup(const CSRMatrix<T, I>& X,
                            const std::vector<const Tensor<T>*>& W,
                            Tensor<T>* Z,
                            const Group18EmbeddingLookupAux& aux) noexcept {
  int n = Z->dim(1);
  T* _Z = Z->data();
  Z->zeros();
  CSR_FOR_EACH_ROW(X, i) {
    CSR_FOR_EACH_COL(X, i) {
      I j = CSR_COL(X);
      T Xij = CSR_VALUE(X);
      int group_id = LLSparseTensor<T, I>::group_18_get_group_id(j);
...
template <typename T, typename I>
void Group18EmbeddingLookupBackward(
    const CSRMatrix<T, I>& X, const std::vector<const Tensor<T>*>& W,
...
template <typename T, typename I>
void Group18SparseEmbeddingLookup(
...
template <typename T, typename I>
void Group18SparseEmbeddingLookupBackward(
    const CSRMatrix<T, I>& X,
...
Group18EmbeddingLookupNode::Group18EmbeddingLookupNode(
  ...
  if (X->shape().is_rank(2) && HasShape(W)) {
    ...
    (void)Group18EmbeddingLookupInferShape(X->shape()[0], group_ids_, Wshape,
                                           &shape_);
  }
}

class Group18EmbeddingLookupOp : public OpImpl {
 private:
  ...
  Group18EmbeddingLookupAux aux_;

 public:
  DEFINE_OP_LIKE(Group18EmbeddingLookupOp);

  void InitForward() override {
    DXCHECK_THROW(!node_->input(0)->need_grad());
    group_ids_ = ((const Group18EmbeddingLookupNode*)node_)->group_ids();
    ...
    DXCHECK_THROW(
        Group18EmbeddingLookupPrepare(X_->row(), group_ids_, Wshape_, &aux_));
  ...
  void Forward() override {
    switch (W_tensor_type_) {
      case TENSOR_TYPE_TSR:
        Group18EmbeddingLookup(*X_, Wtsr_, Z_, aux_);
        break;
      case TENSOR_TYPE_SRM:
        Group18SparseEmbeddingLookup(*X_, Wsrm_, Z_, aux_);
        break;
    }
  }
  ...
  void Backward() override {
    if (gZ_) {
      switch (W_tensor_type_) {
        case TENSOR_TYPE_TSR:
          Group18EmbeddingLookupBackward(*X_, Wtsr_, *Z_, *gZ_, &gW_, aux_);
          break;
        case TENSOR_TYPE_SRM:
          Group18SparseEmbeddingLookupBackward(*X_, Wsrm_, *Z_, *gZ_, &gW_,
                                               aux_);
          break;
      }
    }
      ...
      case TENSOR_TYPE_SRM:
        CSR_FOR_EACH_ROW(*X_, i) {
          CSR_FOR_EACH_COL(*X_, i) {
            int_t j = CSR_COL(*X_);
            int group_id = ll_sparse_tensor_t::group_18_get_group_id(j);
...
GRAPH_NODE_OP_REGISTER(Group18EmbeddingLookup);
...
bool Group18EmbeddingLookup2Prepare(int Xrow, const Shape& W,
                                    const std::vector<int>& group_ids,
                                    Group18EmbeddingLookupAux* aux) {
  if (!W.is_rank(2)) {
...
bool Group18EmbeddingLookup2InferShape(int Xrow, const Shape& W,
                                       const std::vector<int>& group_ids,
                                       Shape* Z) {
  Group18EmbeddingLookupAux aux;
  if (!Group18EmbeddingLookup2Prepare(Xrow, W, group_ids, &aux)) {
    return false;
  }
  *Z = aux.Z;
  return true;
...
template <typename T, typename I>
void Group18EmbeddingLookup2(const CSRMatrix<T, I>& X, const Tensor<T>& W,
                             Tensor<T>* Z,
                             const Group18EmbeddingLookupAux& aux) noexcept {
  int Wrow = W.dim(0);
  int Wcol = W.dim(1);
...
template <typename T, typename I>
void Group18EmbeddingLookup2Backward(
...
template <typename T, typename I>
void Group18SparseEmbeddingLookup2(
    const CSRMatrix<T, I>& X, const SparseRowMatrix<T, I>& W, Tensor<T>* Z,
    const Group18EmbeddingLookupAux& aux) noexcept {
  int Wcol = W.col();
  int n = Z->dim(1);
  T* _Z = Z->data();
...
template <typename T, typename I>
void Group18SparseEmbeddingLookup2Backward(
...
Group18EmbeddingLookup2Node::Group18EmbeddingLookup2Node(
    std::string name, GraphNode* X, GraphNode* W, std::vector<int> group_ids)
    : GraphNode(std::move(name)), group_ids_(std::move(group_ids)) {
  DXCHECK_THROW(X->node_type() == GRAPH_NODE_TYPE_INSTANCE);
  DXCHECK_THROW(X->tensor_type() == TENSOR_TYPE_CSR);
  ...
  if (X->shape().is_rank(2) && !W->shape().empty()) {
    (void)Group18EmbeddingLookup2InferShape(X->shape()[0], W->shape(),
                                            group_ids_, &shape_);
  }
}

class Group18EmbeddingLookup2Op : public OpImpl {
 private:
 ...
 public:
  DEFINE_OP_LIKE(Group18EmbeddingLookup2Op);

  void InitForward() override {
    const std::vector<int>& group_ids =
        ((const Group18EmbeddingLookup2Node*)node_)->group_ids();
    ...
    switch (W_tensor_type_) {
      case TENSOR_TYPE_TSR:
        ...
        DXCHECK_THROW(Group18EmbeddingLookup2Prepare(X_->row(), Wtsr_->shape(),
                                                     group_ids, &aux_));
        break;
      case TENSOR_TYPE_SRM:
        Wsrm_ = GetPtrSRM(Wnode_);
        DXCHECK_THROW(Group18EmbeddingLookup2Prepare(X_->row(), Wsrm_->shape(),
                                                     group_ids, &aux_));
        break;
    }
    Z_ = InitHiddenTSR(node_, aux_.Z);
  ...
  void Forward() override {
    switch (W_tensor_type_) {
      case TENSOR_TYPE_TSR:
        Group18EmbeddingLookup2(*X_, *Wtsr_, Z_, aux_);
        break;
      case TENSOR_TYPE_SRM:
        Group18SparseEmbeddingLookup2(*X_, *Wsrm_, Z_, aux_);
        break;
    }
  }
  ...
  void Backward() override {
    if (gW_) {
      switch (W_tensor_type_) {
        case TENSOR_TYPE_TSR:
          Group18EmbeddingLookup2Backward(*X_, *Wtsr_, *Z_, *gZ_, gW_, aux_);
          break;
        case TENSOR_TYPE_SRM:
          Group18SparseEmbeddingLookup2Backward(*X_, *Wsrm_, *Z_, *gZ_, gW_,
                                                aux_);
          break;
      }
    }
...
GRAPH_NODE_OP_REGISTER(Group18EmbeddingLookup2);
