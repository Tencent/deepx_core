...
/**
 * hdfsGlobStatus - Get list of files/directories for a given
 * glob pattern. hdfsFreeFileInfo should be called to deallocate memory.
 * @param fs The configured filesystem handle.
 * @param pattern The glob pattern(as supported by Hadoop implementation) to
 ...
 * errno is set to non-zero on error or zero on success.
 */
hdfsFileInfo* hdfsGlobStatus(hdfsFS fs, const char* pattern, int* numEntries);
